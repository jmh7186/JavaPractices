<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	<div class="container mt-3">
		...
		<h2>회원 정보</h2>
		<form action="/member/updateProc" onsubmit="return confirmsignup()">
			<div class="mb-3 mt-3">
				<label>아이디:</label> <input type="text" class="form-control" id="idview" name="idview" disabled value="${member.id}">
				<label>아이디:</label> <input type="text" class="form-control" id="id" name="id" hidden readonly value="${member.id}">
			</div>
			<div class="mb-3 mt-3">
				<label>비밀번호:</label>
				<input type="password" class="form-control" id="pwd" name="pwd" disabled readonly value="${member.pwd}">
			...
			<div class="mb-3 mt-3">
				<label>전화번호:</label> <input type="text"
					class="form-control" id="hp" name="hp" disabled value="${member.hp}">
			</div>
			<div class="mb-3 mt-3">
				<label>주소:</label> <input type="text"
					class="form-control" id="adr" name="adr" disabled value="${member.adr}">
			</div>
			<div id="defaultBtns">
	...
	function mbdelete() {
		...
		if(isok) {
			location.href='/member/deleteProc?id=${member.id}';
		}
	}
	function mbupdate () {
